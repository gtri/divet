...
class InstructionSet():
	...
	def __lshift__(self, op):
		#op:= instr_byte, instr_byte_taint 
		self._instruction_composer.append(op)
		self._registers.R = (self._registers.R & 0x10) | ((self._registers.R + 1) & 0x7F)
	...
	def ld_r_r_(instruction, registers, get_reads, data, r, r_):
		if get_reads:
			...
			registers[taintName(r)] = registers[taintName(r_)]
			return []
	...
	def ld_a_ir(instruction, registers, get_reads, data, r):
		if get_reads:
			...
			registers.A_taint = registers[taintName(r)]
	...
	def ld_r_hl(instruction, registers, get_reads, data, r):
		if get_reads:
			...
			registers[r] = data[0][0]
			registers[taintName(r)] = data[0][1]
			return []
	...
	def ld_r_i_d(instruction, registers, get_reads, data, r, i, d):
		if get_reads:
			return [registers[i] + get_8bit_twos_comp(d[0])]
		else:
			registers[r] = data[0][0]
			registers[taintName(r)] = data[0][1]
			return []
	...
	def ld_hl_r(instruction, registers, get_reads, data, r):
		if get_reads:
			...
			return [(registers.H << 8 | registers.L, registers[r], registers[taintName(r)])]
	...
	def ld_i_d_r(instruction, registers, get_reads, data, r, i, d):
		if get_reads:
			...
			return [( registers[i] + get_8bit_twos_comp(d[0]), registers[r], registers[taintName(r)])]
	...
	def ld_hl_n(instruction, registers, get_reads, data, n):
		if get_reads:
			...
			return [(registers.H << 8 | registers.L, n[0], n[1])]
	...
	def ld_i_d_n(instruction, registers, get_reads, data, i, d, n):
		if get_reads:
			...
			return [( registers[i] + get_8bit_twos_comp(d[0]), n[0], n[1])]
	...
	def ld_a_rr(instruction, registers, get_reads, data, r, r2):
		if get_reads:
			...
			registers.A = data[0][0]
			registers.A_taint = data[0][1]
			return []
	...
	def ld_rr_a(instruction, registers, get_reads, data, r, r2):
		if get_reads:
			...
			return [(registers[r] << 8 | registers[r2], registers.A, registers.A_taint)]
	...
	def ld_nn_a(instruction, registers, get_reads, data, n, n2):
		if get_reads:
			...
			return [(n2[0] << 8 | n[0], registers.A, registers.A_taint)]
	...
	def ld_dd_nn(instruction, registers, get_reads, data, r, r2, n, n2):
		if get_reads:
			...
			registers[r] = n2[0]
			registers[taintName(r)] = n2[1]
			registers[r2] = n[0]
			registers[taintName(r2)] = n[1]
			return []
	...
	def ld_D_nn(instruction, registers, get_reads, data, r, n, n2):
		if get_reads:
			...
			registers[r] = n2[0] << 8 | n[0]
			registers[taintName(r)] = n2[1] | n[1]
			return []
	...
	def ld_dd_nn_(instruction, registers, get_reads, data, r, r_, n, n_):
		if get_reads:
			return [n_[0] << 8 | n[0], (n_[0] << 8 | n[0]) + 1]
		else:
			registers[r] = data[1][0]
			registers[taintName(r)] = data[1][1]
			registers[r_] = data[0][0]
			registers[taintName(r_)] = data[0][1]
			return []
	...
	def ld_D_nn_(instruction, registers, get_reads, data, r, n, n2):
		if get_reads:
			return [n2[0] << 8 | n[0], (n2[0] << 8 | n[0]) + 1]
		else:
			registers[r] = data[1][0] << 8 | data[0][0]
			registers[taintName(r)] = data[1][1] | data[0][1]
			return []
	...
	def ld_nn__D(instruction, registers, get_reads, data, r, n, n2):
		if get_reads:
			...
			ad = n2[0] << 8 | n[0]
			return [(ad + 1, registers[r] >> 8, registers[taintName(r)]),
					(ad, registers[r] & 255, registers[taintName(r)])]
	...
	def ld_nn_D(instruction, registers, get_reads, data, r, r2, n, n2):
		if get_reads:
			...
			ad = n2[0] << 8 | n[0]
			return [(ad + 1, registers[r], registers[taintName(r)]),
					(ad, registers[r2], registers[taintName(r2)])]
	...
	def ld_sp_hl(instruction, registers, get_reads, data):
		if get_reads:
			...
			registers.SP_taint = registers.H_taint | registers.L_taint
			return []
	...
	def ld_sp_i(instruction, registers, get_reads, data, i):
		if get_reads:
			...
			registers.SP_taint = registers[taintName(i)]
			return []
	...
	def push_qq(instruction, registers, get_reads, data, q, q2):
		if get_reads:
			...
			stack = registers.SP
			registers.SP -= 2
			return [(stack - 1, registers[q], registers[taintName(q)]), (stack - 2, registers[q2], registers[taintName(q2)])]
	...
	def push_i(instruction, registers, get_reads, data, i):
		if get_reads:
			...
			stack = registers.SP
			registers.SP -= 2
			return [(stack - 1, registers[i] >> 8, registers[taintName(i)]), (stack - 2, registers[i] & 255, registers[taintName(i)])]
	...
	def pop_qq(instruction, registers, get_reads, data, q, q2):
		if get_reads:
			...
			registers.SP += 2
			registers[q2] = data[0][0]
			registers[taintName(q2)] = data[0][1]
			registers[q] = data[1][0]
			registers[taintName(q)] = data[1][1]
			return []
	...
	def pop_i(instruction, registers, get_reads, data, i):
		if get_reads:
			...
			registers.SP += 2
			registers[i] = data[1][0] << 8 | data[0][0]
			registers[taintName(i)] = data[1][1] | data[0][1]
			return []
	...
	def ex_de_hl(instruction, registers, get_reads, data):
		if get_reads:
			...
			registers.E_taint, registers.H_taint = (registers.H_taint, registers.D_taint)
			registers.E, registers.L = (registers.L, registers.E)
			registers.E_taint, registers.L_taint = (registers.L_taint, registers.E_taint)
			return []
	...
	def ex_af_af_(instruction, registers, get_reads, data):
		if get_reads:
			...
			registers.A_taint, registers.A__taint = (registers.A__taint, registers.A_taint)
			registers.F, registers.F_ = (registers.F_, registers.F)
			registers.F_taint, registers.F__taint = (registers.F__taint, registers.F_taint)
			return []
	...
	def exx(instruction, registers, get_reads, data):
		if get_reads:
			...
			registers.B_taint, registers.B__taint = (registers.B__taint, registers.B_taint)
			registers.C, registers.C_ = (registers.C_, registers.C)
			registers.C_taint, registers.C__taint = (registers.C__taint, registers.C_taint)
			registers.D, registers.D_ = (registers.D_, registers.D)
			registers.D_taint, registers.D__taint = (registers.D__taint, registers.D_taint)
			registers.E, registers.E_ = (registers.E_, registers.E)
			registers.E_taint, registers.E__taint = (registers.E__taint, registers.E_taint)
			registers.H, registers.H_ = (registers.H_, registers.H)
			registers.H_taint, registers.H__taint = (registers.H__taint, registers.H_taint)
			registers.L, registers.L_ = (registers.L_, registers.L)
			registers.L_taint, registers.L__taint = (registers.L__taint, registers.L_taint)
			return []
	...
	def ex_sp__hl(instruction, registers, get_reads, data):
		if get_reads:
			...
			h = registers.H
			h_taint = registers.H_taint
			l = registers.L
			l_taint = registers.L_taint
			registers.H = data[1][0]
			registers.H_taint = data[1][1]
			registers.L = data[0][0]
			registers.L_taint = data[0][1]
			return [(registers.SP, l, l_taint), (registers.SP + 1, h, h_taint)]
	...
	def ex_sp__i(instruction, registers, get_reads, data, i):
		if get_reads:
			...
			r = registers[i]
			r_taint = registers[taintName(i)]
			registers[i] = data[1][0] << 8 | data[0][0]
			registers[taintName(i)] = data[1][1] | data[0][1]

			return [(registers.SP, r & 255, r_taint),
					(registers.SP + 1, r >> 8, r_taint)]
	...
	def ldi(instruction, registers, get_reads, data):
		if get_reads:
			...
			de_taint = registers.D_taint | registers.E_taint
			hl = registers.H << 8 | registers.L
			hl_taint = registers.H_taint | registers.L_taint
			bc = registers.B << 8 | registers.C
			bc_taint = registers.B_taint | registers.C_taint
			hl += 1
			if hl > 0xFFFF: hl = 0
			registers.H = hl >> 8
			registers.H_taint = hl_taint
			registers.L = hl & 0xFF
			registers.L_taint = hl_taint
			bc -= 1
			if bc < 0: bc = 0xFFFF
			registers.B = bc >> 8
			registers.B_taint = bc_taint
			registers.C = bc & 0xFF
			registers.C_taint = bc_taint
			de += 1
			if de > 0xFFFF: de = 0
			registers.D = de >> 8
			registers.D_taint = de_taint
			registers.E = de & 0xFF
			registers.E_taint = de_taint
			...
			registers.condition.N = 0
			registers.condition.F3 = (registers.A + data[0][0]) & 0x08
			registers.condition.F5 = (registers.A + data[0][0]) & 0x02
			return [(de_, data[0][0], data[0][1])]
	...
	def add_a_r(instruction, registers, get_reads, data, r):
		if get_reads:
			...
			registers.A_taint = registers.A_taint | registers[taintName(r)]
			return []
	...
	def add_a_n(instruction, registers, get_reads, data, n):
		if get_reads:
			...
			registers.A_taint = registers.A_taint | n[1]
			return []
	...
	def add_a_hl_(instruction, registers, get_reads, data):
		if get_reads:
			...
			registers.A_taint = registers.A_taint | data[0][1]
			return []
	...
	def adc_a_n(instruction, registers, get_reads, data, n):
		if get_reads:
			...
			registers.A = add8(registers.A + registers.condition.C, n[0], registers)
			registers.A_taint = registers.A_taint | n[1]
			return []
	...
	def adc_a_hl_(instruction, registers, get_reads, data):
		if get_reads:
			...
			registers.A = add8(registers.A + registers.condition.C, data[0][0], registers)
			registers.A_taint = registers.A_taint | data[0][1]
			return []
	...
	def sbc_a_n(instruction, registers, get_reads, data, n):
		if get_reads:
			...
			registers.A = subtract8_check_overflow(registers.A - registers.condition.C, n[0], registers)
			registers.A_taint = registers.A_taint | n[1]
			return []
	...
	def sbc_a_hl_(instruction, registers, get_reads, data):
		if get_reads:
			...
			registers.A = subtract8_check_overflow(registers.A - registers.condition.C, data[0][0], registers)
			registers.A_taint = registers.A_taint | data[0][1]
			return []
	...
	def and_a_r(instruction, registers, get_reads, data, r):
		if get_reads:
			...
			a_and_n(registers, registers[r], registers[taintName(r)])
	...
	def and_a_hl_(instruction, registers, get_reads, data):
		if get_reads:
			...
			a_and_n(registers, data[0][0], data[0][1])
			return []
	...
	def or_a_r(instruction, registers, get_reads, data, r):
		if get_reads:
			...
			a_or_n(registers, registers[r], registers[taintName(r)])
			registers.condition.S = (registers.A >> 7)
			return []
	...
	def or_a_hl_(instruction, registers, get_reads, data):
		if get_reads:
			...
			a_or_n(registers, data[0][0], data[0][1])
			return []
	...
	def xor_a_r(instruction, registers, get_reads, data, r):
		if get_reads:
			...
			a_xor_n(registers, registers[r], registers[taintName(r)])
			registers.condition.S = (registers.A >> 7)
			return []
	...
	def xor_a_hl_(instruction, registers, get_reads, data):
		if get_reads:
			...
			a_xor_n(registers, data[0][0], data[0][1])
			return []
	...
	def inc_hl_(instruction, registers, get_reads, data):
		if get_reads:
			...
			new = add8(data[0][0], 1, registers, C=False )
			taint = data[0][1]
			return [(registers.HL, new, taint)]
	...
	def dec_hl_(instruction, registers, get_reads, data):
		if get_reads:
			...
			new = subtract8(data[0][0], 1, registers,
									 PV=False)
			taint = data[0][1]
			if data[0][0] == 0x80:
				registers.condition.PV = 1
	...
	def dec_i_(instruction, registers, get_reads, data, i, d):
		if get_reads:
			...
			if data[0][0] == 0x80:
				...
				registers.condition.PV = 0
			return [(registers[i] + get_8bit_twos_comp(d[0]), new, taint)]
	...
	def daa(instruction, registers, get_reads, data):
		if get_reads:
			...
			c = registers.condition.C
			h = registers.condition.H
			a = registers.A
			ah = (a >> 4) & 0xF
			al = a & 0xF
			r09 = range(0x00,0x0A)
			...
			r9f = range(0x09,0x10)
			raf = range(0x0A,0x10)
			if c == 0 and h == 0 and ah in r09 and al in r09:
				registers.A = (a + 0x00) & 0xFF
				registers.condition.C = 0
				registers.condition.Z = (registers.A == 0)
				registers.condition.PV = parity(registers.A)
			elif c == 0 and h == 0 and ah in r08 and al in raf:
				registers.A = (a + 0x06) & 0xFF
				registers.condition.C = 0
				registers.condition.Z = (registers.A == 0)
				registers.condition.PV = parity(registers.A)
			elif c == 0 and h == 1 and ah in r09 and al in r03:
				registers.A = (a + 0x06) & 0xFF
				registers.condition.C = 0
				registers.condition.Z = (registers.A == 0)
				registers.condition.PV = parity(registers.A)
			elif c == 0 and h == 0 and ah in raf and al in r09:
				registers.A = (a + 0x60) & 0xFF
				registers.condition.C = 1
				registers.condition.Z = (registers.A == 0)
				registers.condition.PV = parity(registers.A)
			elif c == 0 and h ==0 and ah in r9f and al in raf:
				registers.A = (a + 0x66) & 0xFF
				registers.condition.C = 1
				registers.condition.Z = (registers.A == 0)
				registers.condition.PV = parity(registers.A)
			elif c == 0 and h ==1 and ah in raf and al in r03:
				registers.A = (a + 0x66) & 0xFF
				registers.condition.C = 1
				registers.condition.Z = (registers.A == 0)
				registers.condition.PV = parity(registers.A)
			elif c == 1 and h == 0 and ah in r02 and al in r09:
				registers.A = (a + 0x60) & 0xFF
				registers.condition.C = 1
				registers.condition.Z = (registers.A == 0)
				registers.condition.PV = parity(registers.A)
			elif c == 1 and h == 0 and ah in r02 and al in raf:
				registers.A = (a + 0x66) & 0xFF
				registers.condition.C = 1
				registers.condition.Z = (registers.A == 0)
				registers.condition.PV = parity(registers.A)
			elif c == 1 and h == 1 and ah in r03 and al in r03:
				registers.A = (a + 0x66) & 0xFF
				registers.condition.C = 1
				registers.condition.Z = (registers.A == 0)
				registers.condition.PV = parity(registers.A)
			elif c == 0 and h == 0 and ah in r09 and al in r09:
				registers.A = (a + 0x00) & 0xFF
				registers.condition.C = 0
				registers.condition.Z = (registers.A == 0)
				registers.condition.PV = parity(registers.A)
			elif c == 0 and h == 1 and ah in r08 and al in r6f:
				registers.A = (a + 0xFA) & 0xFF
				registers.condition.C = 0
				registers.condition.Z = (registers.A == 0)
				registers.condition.PV = parity(registers.A)
			elif c == 1 and h == 0 and ah in r7f and al in r09:
				registers.A = (a + 0xA0) & 0xFF
				registers.condition.C = 1
				registers.condition.Z = (registers.A == 0)
				registers.condition.PV = parity(registers.A)
			elif c == 1 and h == 1 and ah in r67 and al in r6f:
				registers.A = (a + 0x9A) & 0xFF
				registers.condition.C = 1
				registers.condition.Z = (registers.A == 0)
				registers.condition.PV = parity(registers.A)
			return []
	...
	def add16_hl(instruction, registers, get_reads, data, reg):
		if get_reads:
			...
			taint = registers.HL_taint | registers[taintName(reg)]
	...
	def adc16_hl(instruction, registers, get_reads, data, reg):
		if get_reads:
			...
			registers.HL_taint = registers.HL_taint | registers[taintName(reg)]
			return []
	...
	def add16_i_pp(instruction, registers, get_reads, data, i, r):
		if get_reads:
			...
			taint = registers[taintName(i)] | registers[taintName(r)]
	...
	def rlc_hl_(instruction, registers, get_reads, data):
		if get_reads:
			...
			val = rotate_left_carry(registers, data[0][0])
			taint = data[0][1]
			set_f5_f3(registers, val)
			return [(registers.HL, val, taint)]
	...
	def rlc_i_d(instruction, registers, get_reads, data, i, d):
		if get_reads:
			...
			new = rotate_left_carry(registers, data[0][0])
			taint = data[0][1]
	...
	def rl_hl(instruction, registers, get_reads, data):
		if get_reads:
			...
			val = rotate_left(registers, data[0][0])
			taint = data[0][1]
			set_f5_f3(registers, val)
			return [(registers.HL, val, taint)]
	...
	def rl_i(instruction, registers, get_reads, data, i, d):
		if get_reads:
			...
			taint = data[0][1]
			set_f5_f3(registers, val)
			return [(registers[i] + get_8bit_twos_comp(d[0]), val, taint)]
	...
	def rrc_hl_(instruction, registers, get_reads, data):
		if get_reads:
			...
			val = rotate_right_carry(registers, data[0][0])
			taint = data[0][1]
			set_f5_f3(registers, val)
			return [(registers.HL, val, taint)]
	...
	def rrc_i_d(instruction, registers, get_reads, data, i, d):
		if get_reads:
			...
			new = rotate_right_carry(registers, data[0][0])
			taint = data[0][1]
			set_f5_f3(registers, new)
			return [(registers[i] + get_8bit_twos_comp(d[0]), new, taint)]
	...
	def rr_hl(instruction, registers, get_reads, data):
		if get_reads:
			...
			val = rotate_right(registers, data[0][0])
			taint = data[0][1]
			set_f5_f3(registers, val)
			return [(registers.HL, val, taint)]
	...
	def sla_hl_(instruction, registers, get_reads, data):
		if get_reads:
			...
			val = shift_left(registers, data[0][0])
			taint = data[0][1]
			set_f5_f3(registers, val)
			return [(registers.HL, val, taint)]
	...
	def sla_i_d(instruction, registers, get_reads, data, i, d):
		if get_reads:
			...
			new = shift_left(registers, data[0][0])
			taint = data[0][1]
			set_f5_f3(registers, new)
			return [(registers[i] + get_8bit_twos_comp(d[0]), new, taint)]
	...
	def sra_r(instruction, registers, get_reads, data, r):
		if get_reads:
			...
			registers[r] = shift_right_arithmetic(registers, registers[r])
			set_f5_f3(registers, registers[r])
			return []
	...
	def sra_hl(instruction, registers, get_reads, data):
		if get_reads:
			...
			val = shift_right_arithmetic(registers, data[0][0])
			taint = data[0][1]
			set_f5_f3(registers, val)
			return [(registers.HL, val, taint)]
	...
	def srl_hl(instruction, registers, get_reads, data):
		if get_reads:
			...
			val = shift_right_logical(registers, data[0][0])
			taint = data[0][1]
			set_f5_f3(registers, val)
			return [(registers.HL, val, taint)]
	...
	def rld(instruction, registers, get_reads, data):
		if get_reads:
			...
			a = (data[0][0] >> 4) | (registers.A & 0xF0)
			hl = ((registers.HL << 4) | (registers.A & 0x0f)) & 0xFF
			hl_taint = registers.HL_taint | registers.A_taint
			registers.A = a
			registers.A_taint = data[0][1] | registers.A_taint
			...
			return [(registers.HL, hl, hl_taint)]
	...
	def rrd(instruction, registers, get_reads, data):
		if get_reads:
			...
			a = (data[0][0] & 0x0F) | (registers.A & 0xF0)
	...
	def bit_hl(instruction, registers, get_reads, data, bit):
		if get_reads:
			...
			registers.condition.Z = (data[0][0] & (0x01 << bit)) == 0
			registers.condition.H = 1
			registers.condition.N = 0
			registers.condition.PV = registers.condition.Z
			if bit == 7:
				registers.condition.S = (data[0][0] & (0x01 << bit))
			set_f5_f3(registers, data[0][0])
			return []
	...
	def bit_i(instruction, registers, get_reads, data, i, bit, d):
		if get_reads:
			...
			set_f5_f3(registers, (registers[i]+get_8bit_twos_comp(d[0])) >> 8)
			return []
	...
	def set_r(instruction, registers, get_reads, data, bit, reg):
		if get_reads:
			...
			regTaint = taintName(reg)
			registers[regTaint] = registers[regTaint] | instruction.bytes[1][1]
			return []
	...
	def set_hl(instruction, registers, get_reads, data, bit):
		if get_reads:
			...
			val = data[0][0] | (0x01 << bit)
			taint = instruction.bytes[1][1] | data[0][1]
			return [(registers.HL, val, taint)]
	...
	def jp(instruction, registers, get_reads, data, n, n2):
		if get_reads:
			...
			registers.PC = n2[0] << 8 | n[0]
			registers.PC_taint = n2[1] | n[1]
			return []
	...
	def jr(instruction, registers, get_reads, data, n):
		if get_reads:
			...
			offset_pc(registers, n[0])
			registers.PC_taint = registers.PC_taint | n[1]
			return []
	...
	def jp_r(instruction, registers, get_reads, data, r):
		if get_reads:
			...
			registers.PC_taint = registers[taintName(r)]
			return []
	...
	def call(instruction, registers, get_reads, data, n, n2):
		if get_reads:
			...
			sp = registers.SP
			pc = registers.PC
			pc_taint = registers.PC_taint
			registers.SP = dec16(registers.SP)
			registers.SP = dec16(registers.SP)
			registers.PC = n2[0] << 8 | n[0]
			registers.PC_taint = n2[1] | n[1]
			return [(sp - 1, pc >> 8, pc_taint),
					(sp - 2, pc & 0xFF, pc_taint)]
	...
	def ret(instruction, registers, get_reads, data):
		if get_reads:
			...
			registers.SP = inc16(registers.SP)
			registers.PC = data[1][0] << 8 | data[0][0]
			registers.PC_taint = data[1][1] | data[0][1]
			return []
	...
	def ret_c(instruction, registers, get_reads, data, reg, reg_name, val):
		if get_reads:
			...
			if len(data) == 2:
				...
				registers.SP = inc16(registers.SP)
				registers.PC = data[1][0] << 8 | data[0][0]
				registers.PC_taint = data[1][1] | data[1][1]
	...
	def rst_p(instruction, registers, get_reads, data, p):
		if get_reads:
			...
			registers.SP = dec16(registers.SP)
			registers.PC = p
			registers.PC_taint = registers.PC_taint | instruction.bytes[0][1]
			return [(sp - 1, pc >> 8, pc_taint),
					(sp - 2, pc & 0xFF, pc_taint)]
	...
	def in_a_n(instruction, registers, get_reads, data, n):
		if get_reads:
			address = n[0] | (registers.A << 8) # registers.C | (registers.B << 8)
	...
	def in_r_c(instruction, registers, get_reads, data, r):
		if get_reads:
			...
			registers.condition.S = data[0][0] & 0x80
			registers.condition.Z = data[0][0] == 0
			registers.condition.H = 0
			registers.condition.PV = parity(data[0][0])
			registers.condition.N = 0
			if r == "F":
				return []
			registers[r] = data[0][0]
	...
	def ini(instruction, registers, get_reads, data):
		if get_reads:
			...
			return [(dec16(registers.HL), data[0][0], data[0][1])]
	...
	def out_a_n(instruction, registers, get_reads, data, n):
		if get_reads:
			...
			address = n[0] | (registers.A << 8)
			return [(address+0x10000, registers.A, registers.A_taint)]
	...
	def out_r_c(instruction, registers, get_reads, data, r):
		if get_reads:
			...
			if r == "F":
				return []
			return [(registers.BC+0x10000, registers[r], registers[taintName(r)])]
	...
	def outi(instruction, registers, get_reads, data):
		if get_reads:
			...
			registers.condition.N = 1
			registers.condition.Z = registers.B == 0
			return [(address+0x10000, data[0][0], data[0][1])]
	...
	def otir(instruction, registers, get_reads, data):
		if get_reads:
			...
			return [(registers.BC+0x10000, data[0][0], data[0][1])]
