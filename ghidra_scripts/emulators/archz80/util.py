...
def a_and_n(registers, n, n_taint):
	registers.A = registers.A & n
	registers.A_taint = registers.A_taint | n_taint
	registers.condition.H = 1
	registers.condition.N = 0
...
def a_or_n(registers, n, n_taint):
	registers.A = registers.A | n
	registers.A_taint = registers.A_taint | n_taint
	registers.condition.H = 0
	registers.condition.N = 0
...
def a_xor_n(registers, n, n_taint):
	registers.A = registers.A ^ n
	registers.A_taint = registers.A_taint | n_taint
	registers.condition.H = 0
	registers.condition.N = 0
...
def shift_right_arithmetic(registers, n):
	c = n & 0x01
	v = (n&0x80) | (n >> 1)
	registers.condition.S = v >> 7
	registers.condition.Z = (v == 0)
