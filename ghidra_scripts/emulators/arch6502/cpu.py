...
class Registers:
	...
	def getState(self):
		...
		if self.yt: taint	|= 4
		if self.st: taint	|= 8
		if self.pct: taint	|= 16
		if self.pt: taint	|= 32
		return "%s%s%s%s%s%s%s%s" % (chr(self.a), chr(self.x), chr(self.y), chr(self.s), chr(self.pc >> 8), chr(self.pc & 0xFF), chr(self.p), chr(taint))
	...
	def setState(self,state):
		...
		self.y = ord(state[2])
		self.s = ord(state[3])
		self.pc = (ord(state[4])<<8) | ord(state[5])
		self.p = ord(state[6])
		taint = ord(state[7])
		self.at	= bool(taint & 1)
		self.xt	= bool(taint & 2)
		self.yt	= bool(taint & 4)
		self.st	= bool(taint & 8)
		self.pct = bool(taint & 16)
		self.pt	= bool(taint & 32)
	...
	def getRegistersDefinition(self):
		...
		["General",	[["a",  "A",  8],
					 ...
					 ["s",  "S",  8],
					 ["pc", "PC", 16]]],
		["Flags",	[["N", "<HTML><U>N</U>egative</HTML>",		1],
					 ["V", "<HTML>O<U>v</U>erflow</HTML>",		1],
					 ["B", "<HTML><U>B</U>reak Command</HTML>",	1],
					 ["D", "<HTML><U>D</U>ecimal Mode</HTML>",	1],
					 ["I", "<HTML><U>I</U>RQ Disable</HTML>",	1],
					 ["Z", "<HTML><U>Z</U>ero</HTML>",			1],
					 ["C", "<HTML><U>C</U>arry</HTML>",			1]]],
		]
	...
	def getRegister(self, regName):
		if regName in self.flagBit:
			return self.getFlag(regName), self.pt
		else:
			regName = regName.lower()
			taintName = "%st" % regName
			return getattr(self, regName,0),getattr(self, taintName,0)
			
	def setRegister(self, regName, value, taint):
		regName = regName.upper()
		if regName in self.flagBit:
			self.setFlag(regName, value)
			self.pt = self.pt | taint
		elif regName in self.regNames:
			regName = regName.lower()
			taintName = "%st" % regName
			setattr(self, regName, value)
			setattr(self, taintName, taint)
		else:
			raise ValueError,"Bad Register name: %s" % regName
			

class CPU:
	def __init__(self, registers, memory, pc=None, stack_page=0x1, magic=0xee):
		"""
		Parameters
		----------
		registers: an instance of Registers
		memory: an instance of MMU/EMulatedMemory
		(old)mmu: An instance of MMU
	...
	def reset(self):
		self.r.reset()
		self.r.pc, self.r.pct = self.interruptAddress('RESET')
		self.running = True

	def irq(self):
		if self.r.getFlag("I"):
			return
		self.stackPushWord(self.r.pc,self.r.pct)
		self.stackPushWord(self.r.p, self.r.pt)
		self.r.setFlag("I")
		self.r.pc, self.r.pct = self.interruptAddress('IRQ')
		
	def nmi(self):
		self.stackPushWord(self.r.pc,self.r.pct)
		self.stackPushWord(self.r.p, self.r.pt)
		self.r.pc, self.r.pct = self.interruptAddress('NMI')

	def step(self):
		self.cc = 0
		opcode, t = self.nextByte()
		self.ops[opcode]()
	...
	def nextByte(self):
		v, t = self.mmu.readByte(self.r.pc)
		self.r.pc += 1
		return v, t

	def nextWord(self):
		low, t1 = self.nextByte()
		high, t2 = self.nextByte()
		return (high << 8) + low, t1 | t2

	def stackPush(self, v, t):
		self.mmu.write(self.stack_page*0x100 + self.r.s, v, t)
		self.r.s = (self.r.s - 1) & 0xff

	def stackPushWord(self, v, t):
		self.stackPush(v >> 8, t)
		self.stackPush(v & 0xff, t)

	def stackPop(self):
		v, t = self.mmu.readByte(self.stack_page*0x100 + ((self.r.s + 1) & 0xff))
		self.r.s = (self.r.s + 1) & 0xff
		return v, t
	...
	def interruptAddress(self, i):
		value, taint = self.mmu.readWord(self.interrupts[i])
		return value, taint
	...
	def zx_a(self):
		v, t = self.nextByte()
		return (v + self.r.x) & 0xff, t

	def zy_a(self):
		v, t = self.nextByte()
		return (v + self.r.y) & 0xff, t
	...
	def i_a(self):
		...
		jv, jt = self.mmu.readByte(j)
		iv, it = self.mmu.readByte(i)
		return ((jv << 8) + iv) & 0xffff, jt | it

	def ix_a(self):
		v, t = self.nextByte()
		i = (v + self.r.x) & 0xff
		iv1, it1 = self.mmu.readByte((i + 1) & 0xff)
		iv, it = self.mmu.readByte(i)
		return ((iv1 << 8) + iv) & 0xffff, it1 | it
	...
	def z(self):
		return self.mmu.readByte(self.z_a()[0])

	def zx(self):
		return self.mmu.readByte(self.zx_a()[0])

	def zy(self):
		return self.mmu.readByte(self.zy_a()[0])

	def a(self):
		return self.mmu.readByte(self.a_a()[0])

	def ax(self):
		return self.mmu.readByte(self.ax_a()[0])

	def ay(self):
		return self.mmu.readByte(self.ay_a()[0])

	def i(self):
		return self.mmu.readByte(self.i_a()[0])

	def ix(self):
		return self.mmu.readByte(self.ix_a()[0])

	def iy(self):
		return self.mmu.readByte(self.iy_a()[0])
	...
	def AND(self, v, t):
		...
		self.r.at = self.r.at | t
		self.r.ZN(self.r.a)

	def ASL(self, a, t=None):
		if a == 'a':
			v = self.r.a << 1
			self.r.a = v & 0xff
		else:
			v, vt = self.mmu.readByte(a)
			v = v << 1
			self.mmu.write(a, v, vt)

		self.r.setFlag('C', v > 0xff)
		self.r.ZN(v & 0xff)

	def BIT(self, v, t=None):
	...
	def BRK(self, v=None, t=None):
		self.r.setFlag('B')
		self.stackPushWord(self.r.pc + 1, self.r.pct)
		self.stackPush(self.r.p, self.r.pt)
		self.r.setFlag('I')
		self.r.pc, self.r.pct = self.interruptAddress('BRK')
	...
	def DEC(self, a, t=None):
		v, t = self.mmu.readByte(a)
		v = (v - 1) & 0xff
		self.mmu.write(a, v, t)
		self.r.ZN(v)

	def DEX(self, v=None, t=None):
		self.r.x = (self.r.x - 1) & 0xff
		self.r.ZN(self.r.x)

	def DEY(self, v=None, t=None):
	...
	def INC(self, a, t=None):
		v, t = self.mmu.readByte(a)
		v = (v + 1) & 0xff
		self.mmu.write(a, v, t)
		self.r.ZN(v)

	def INX(self, v=None, t=None):
		self.r.x = (self.r.x + 1) & 0xff
		self.r.ZN(self.r.x)

	def INY(self, v=None, t=None):
		self.r.y = (self.r.y + 1) & 0xff
		self.r.ZN(self.r.y)
	...
	def NOP(self, v=None, t=None):
		pass

	def ORA(self, v, t):
		self.r.a = self.r.a | v
		self.r.at = t | self.r.at
		self.r.ZN(self.r.a)
	...
	def P(self, v, t=None):
		...
		if a == "PH":
			self.stackPush(getattr(self.r, r),getattr(self.r,"%st"%r))
		else:
			m, mt = self.stackPop()
			setattr(self.r, r, m)
			setattr(self.r,"%st"%r,mt)
	...
	def RTI(self, v=None, t=None):
		self.r.p, self.r.pt = self.stackPop()
		self.r.pc, self.r.pct = self.stackPopWord()
		self.r.clearFlag("I")

	def RTS(self, v=None, t=None):
		v, t = self.stackPopWord()
		self.r.pc = (v + 1) & 0xffff
		self.r.pct = t
	...
	def STA(self, a, t=None):
		self.mmu.write(a, self.r.a, self.r.at)

	def STX(self, a, t=None):
		self.mmu.write(a, self.r.x, self.r.xt)

	def STY(self, a, t=None):
		self.mmu.write(a, self.r.y, self.r.yt)
	...
	def T(self, a, t=None):
		...
		setattr(self.r, "%st" % d, getattr(self.r, "%st" % s))
		if d != 's':
	...
	def AAC(self, v, t):	# ANC
		self.AND(v, t)
		self.r.setFlag('C', self.r.getFlag('N'))

	def AAX(self, a, t):	# SAX, AXS
		r = self.r.a & self.r.x
		self.mmu.write(a, r, t | self.r.at | self.r.xt)
	...
	def ASR(self, v, t):	# ALR
		self.AND(v,t)
		self.LSR('a')

	def ATX(self, v, t):	# LXA, OAL
		self.AND(v,t)
		self.T(('a', 'x'))

	def AXA(self, a, t=None):	# SHA
		"""
		...
		self.mmu.write(a, v, self.r.at | self.r.xt)

	def AXS(self, v, t):	# SBX, SAX
		o = self.r.a & self.r.x
		self.r.x = (o - v) & 0xff
		self.r.xt = self.r.at | self.r.xt | t

		self.r.setFlag('C', v <= o)
		self.r.ZN(self.r.x)

	def DCP(self, a, t=None):	# DCM
		self.DEC(a)
		self.CMP(self.mmu.readByte(a))

	def ISC(self, a, t=None):	# ISB, INS
		self.INC(a)
		v, t = self.mmu.readByte(a)
		self.SBC(v,t)

	def KIL(self, v=None, t=None):	# JAM, HLT
		self.running = False

	def LAR(self, v, t):	# LAE, LAS
		self.r.a = self.r.x = self.r.s = self.r.s & v
		self.r.at = self.r.xt = self.r.st = self.r.st | t
		self.r.ZN(self.r.a)

	def LAX(self, v, t):
		self.r.a = self.r.x = v
		self.r.at = self.r.xt = t
		self.r.ZN(self.r.a)

	def RLA(self, a, t=None):
		self.ROL(a)
		v, t = self.mmu.readByte(a)
		self.AND(v, t)

	def RRA(self, a, t=None):
		self.ROR(a)
		v, t = self.mmu.readByte(a)
		self.ADC(v, t)

	def SLO(self, a, t=None):	# ASO
		self.ASL(a)
		v, t = self.mmu.readByte(a)
		self.ORA(v, t)

	def SRE(self, a, t=None):	# LSE
		self.LSR(a)
		v, t = self.mmu.readByte(a)
		self.EOR(v, t)

	def SXA(self, a, t):	# SHX, XAS
		# See AXA
		o = (a - self.r.y) & 0xffff
		low = o & 0xff
		high = o >> 8
		...
		v = self.r.x & (high + 1)
		self.mmu.write(a, v, t | self.r.yt | self.r.xt)

	def SYA(self, a, t):	# SHY, SAY
		# See AXA
		o = (a - self.r.x) & 0xffff
		low = o & 0xff
		high = o >> 8
		...
		v = self.r.y & (high + 1)
		self.mmu.write(a, v, t | self.r.xt | self.r.yt)

	def XAA(self, v, t):	# ANE
		"""
		...
		self.r.at = self.r.at | self.r.xt | t
		self.r.ZN(self.r.a)

	def XAS(self, a, t):	# SHS, TAS
		...
		v = self.r.s & (high + 1)
		self.mmu.write(a, v, t | self.r.yt | self.r.st)
